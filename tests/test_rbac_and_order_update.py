...
def test_role_change_requires_admin():
    ...
    r = client.post('/users', json={'name': 'Admin', 'role': 'admin'})
    assert r.status_code == 201
    admin_id = r.json()['id']

    r2 = client.post('/users', json={'name': 'Regular', 'role': 'user'})
    assert r2.status_code == 201
    user_id = r2.json()['id']

    # regular user cannot change roles (before promotion)
    token_regular = client.post('/auth/login', json={'user_id': user_id}).json()['access_token']
    ...
    token = client.post('/auth/login', json={'user_id': admin_id}).json()['access_token']
...
def test_order_update_authorization():
    ...
    r1 = client.post('/users', json={'name': 'Owner'})
    owner_id = r1.json()['id']
    r2 = client.post('/users', json={'name': 'Other'})
    other_id = r2.json()['id']
    r3 = client.post('/users', json={'name': 'Admin2', 'role': 'admin'})
    admin_id = r3.json()['id']
    ...
    # owner updates order
    token_owner = client.post('/auth/login', json={'user_id': owner_id}).json()['access_token']
    ...
    token_other = client.post('/auth/login', json={'user_id': other_id}).json()['access_token']
    resp2 = client.put(f'/orders/{order_id}', json={'amount':'7.00'}, headers={'Authorization': f'Bearer {token_other}'})
    assert resp2.status_code == 403

    # admin can update
    token_admin = client.post('/auth/login', json={'user_id': admin_id}).json()['access_token']
